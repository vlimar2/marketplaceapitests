...
var ContactusqueryService = require('../services/contactusqueryService')
...
describe('Testes na Api de retorno de quantidade de registros de fale conosco do power BI', function () {
    ...
    it('Deve obter retorno e acesso a quantidade de registros de fale conosco do power BI', function () {

        var contactusqueryService = new ContactusqueryService(this);

        return contactusqueryService.getcontactusquery(token).then(function (response) {
    ...
    it('Não deve obter retorno sem autenticação', function () {
        ...
        return contactusqueryService.getcontactusquery().then(function (response) {
    ...
    it('Não deve obter retorno com autenticação que não seja de admin', function () {
        ...
        return authService.authParticipant(config.USERNAME,config.PASS, config.CLIENT_ID).then(function (response) {
            ...
            return contactusqueryService.getcontactusquery(response.body.access_token).then(function (response) {
                expect(response, 'Deve impedir o acesso a informações do power BI').to.have.status(config.util.HTTP.UNAUTHORIZED);
            });//.catch(function (response) {
           // });
        });
